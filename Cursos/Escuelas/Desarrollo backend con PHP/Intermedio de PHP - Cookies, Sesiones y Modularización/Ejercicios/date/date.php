...
/**
 * Añade la cantidad de tiempo indicada a un timestamp, si indicas "now" tomará la fecha actual,
 * como segundo parámetro recibe la cantidad de tiempo a añadir en español,
 * por ejemplo, "1 año", "7 dias", "4 semanas", etc... - RetaxMaster
 *
 * @param string $timestamp
...
function add_time(string $timestamp, string $timeToAdd): string
{
    ...
    foreach ($timeToAdd as $key => $value) {
        $timeToAdd[$key] = translate_time($value, false);
    }
 ...
 * @param string $h1
 * @param string $h2
 * @param boolean $enable_distant_hours
 * @return integer
 */
